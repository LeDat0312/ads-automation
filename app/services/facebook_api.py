...
def pull_facebook_data_with_time_range(
    access_token: str,
    ad_account_ids: List[str],
    ...
    """
    Kéo dữ liệu Insights level=ad từ Facebook API với time_range cụ thể
    Sử dụng time_range với since/until thay vì date_preset
    
    Args:
        access_token: Facebook access token
        ad_account_ids: List account IDs
    ...
    fields = [
        ...
        'spend', 'impressions', 'reach', 'frequency', 'clicks', 'ctr', 'cpc',
        'cost_per_initiate_checkout', 'cost_per_purchase',
        'cost_per_action_type', 'actions', 'action_values', 'date_start'
    ]
    fields_string = ','.join(fields)
    
    # Convert dates to timezone-aware datetime (Asia/Ho_Chi_Minh)
    tz = timezone(timedelta(hours=7))
    since_dt = datetime.combine(date_from, datetime.min.time()).replace(tzinfo=tz)
    until_dt = datetime.combine(date_to, datetime.max.time()).replace(tzinfo=tz)
    
    # Format dates for Facebook API (YYYY-MM-DD)
    since = since_dt.strftime('%Y-%m-%d')
    until = until_dt.strftime('%Y-%m-%d')
    
    # Create time_range JSON
    time_range_json = json.dumps({"since": since, "until": until})
    time_range_param = quote(time_range_json)
    
    logger.info(f"📅 Pulling data với time_range: since={since}, until={until} (inclusive)")
    
    all_rows = []
    
    for account_id in ad_account_ids:
        try:
            logger.info(f"Đang kéo dữ liệu cho tài khoản: {account_id} (từ {since} đến {until})")
            ...
            while True:
                ...
                if next_url:
                    ...
                    # Tạo URL với time_range
                    url = (
                        f"{FB_GRAPH_API_BASE}/{account_id}/insights"
                        f"?level=ad"
                        f"&fields={fields_string}"
                        f"&time_range={time_range_param}"
                        f"&breakdown=day"  # QUAN TRỌNG: breakdown=day để có date_start
                        f"&limit=1000"
                        f"&access_token={access_token}"
                        f"&action_report_time=conversion"
                        f"&use_unified_attribution_setting=true"
                        f"&action_attribution_windows=1d_click,7d_click,1d_view,7d_view"
                    )
                ...
                if not data or not isinstance(data, list) or len(data) == 0:
                    if page_count == 1:
                        logger.warning(f"⚠️ Tài khoản {account_id} không có dữ liệu insights từ {since} đến {until}.")
                    break
                
                logger.info(f"   📊 Page {page_count}: Nhận được {len(data)} ads từ API...")
                
                for item in data:
                    # Parse date_start từ breakdown=day
                    date_start_str = item.get('date_start', '')
                    if date_start_str:
                        try:
                            # Parse date_start (YYYY-MM-DD)
                            item_date = datetime.strptime(date_start_str, '%Y-%m-%d').date()
                        except:
                            item_date = date_from  # Fallback
                    else:
                        item_date = date_from  # Fallback
                    ...
                    # Fallback logic (giống pull_facebook_data)
                    if msg_started == 0 and spend > 0:
                        all_msg_keys = [k for k in act_map.keys() 
                                       if 'messaging_conversation_started' in k.lower() 
                                       and '_unique' not in k.lower()]
                    ...
                    try:
                        ...
                        campaign_type = 'ECOMMERCE'  # Default
                    
                    # Tạo row data với date từ date_start
                    row = {
                        'account_name': item.get('account_name', ''),
                        'account_id': item.get('account_id', ''),
                        'campaign_name': campaign_name,
                        ...
                        'post_comments': comments,
                        'messaging_conversations_started': messages,
                        'date': datetime.combine(item_date, datetime.min.time()),  # Use date from date_start
                        'date_preset': f"custom_{since}_{until}",
                    }
    ...
    logger.info(f"✅ Đã lấy {len(all_rows)} ads từ Facebook API (từ {since} đến {until})")
    return all_rows
